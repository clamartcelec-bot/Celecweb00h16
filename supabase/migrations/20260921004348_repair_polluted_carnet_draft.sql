...
1. Problem repaired
  - One Telegram draft stored the model reasoning text in `description` and in
    ...
    answer was buried at the end of that same text.

2. Fix
  - The genuine JSON object (which starts at the last `{"title"`) is extracted, kept as one
    line, and parsed.
...
WITH target AS (
  SELECT id,
         regexp_replace(
           substring(raw_data->>'raw_response' from '\{"title".*\}'),
           E'[\n\r\t]+', ' ', 'g'
         ) AS payload
  FROM photos
